...
function () {
  function GhostSearch(args) {
    ...
    this.check = false;
    var defaults = {
      url: 'https://sashinexists.com', //"http://localhost:2368" for development, "https://sashinexists.com" for production
      ...
      template: function template(result) {
        ...
        if (result.tags[1].name === "Book") {
          return BOOK_RESULT_TEMPLATE(url, result);
        } else if (result.tags[0].name === "Quote") {
          return QUOTE_RESULT_TEMPLATE(url, result);
        } else if (result.tags[0].name === "Letter") {
          return LETTER_RESULT_TEMPLATE(url, result);
        }
        return SEARCH_RESULT_TEMPLATE(url, result);
      },
      trigger: 'focus',
      options: {
        keys: ['title', 'plaintext', 'tag.name'],
        limit: 10,
        threshold: -3500,
        allowTypo: false
      },
      api: {
        ...
        parameters: {
          limit: 'all',
          fields: ['title', 'slug', 'plaintext', 'feature_image', 'excerpt','custom_excerpt','published_at'],
          filter: '',
          include: 'tags',
          order: '',
          formats: 'plaintext',
      ...
      on: {
        ...
        afterDisplay: function afterDisplay(results) {setTimeout(RENDER_SEARCH(),3000)},
        beforeFetch: function beforeFetch() {clearTimeout();},
